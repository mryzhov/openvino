...
void GNAGraphCompiler::AffinePrimitive(InferenceEngine::CNNLayerPtr layer, bool isDiag) {
    ...
    if (num_padding == 0) {
        if (!transpose) {
            ...
            gnamem->getQueue(REGION_RO)->push_initializer(layer, ptr_weights, weightable._weights->byteSize(), [=](void* data, size_t size) {
    ...
    if (LayerInfo(prevLayer).isInput()) {
        if (0 == inputDesc->bytes_allocated_for_input[prevLayer->name]) {
            ...
            if (!connectTo) {
                ...
                gnamem->getQueue(REGION_INPUTS)->push_value(layer, &inputDesc->getPtrInputsGlobal(prevLayer->name).front(),
                                   static_cast<uint8_t>(0),
                                   num_data_bytes_in,
                                   64);
            }
        ...
        if (connectTo) {
            gnamem->getQueue(REGION_AUTO)
                ->bind_ptr(layer, ptr, &inputDesc->getPtrInputsGlobal(prevLayer->name).front(), offset, ALIGN(num_data_bytes_in, 64));
        } else {
            gnamem->getQueue(REGION_AUTO)
                ->bind_ptr(layer, &inputDesc->getPtrInputsGlobal(prevLayer->name).front(), ptr, offset, ALIGN(num_data_bytes_in, 64));
        }
