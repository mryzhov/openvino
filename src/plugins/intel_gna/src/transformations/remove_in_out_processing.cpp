...
inline bool is_skip_operation(const std::shared_ptr<ov::Node>& node) {
    // Skipping Reshape with one consumer only
...
bool RemoveInputsProcessing::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    for (const auto& param_node : model->inputs()) {
        for (auto& param_target : param_node.get_target_inputs()) {
            std::shared_ptr<ov::Node> target_node = graph_utils::get_next_node_skipping_certain(param_target.get_node()->shared_from_this(),
                                                                           is_skip_operation);
            std::set<std::shared_ptr<ov::Node>> target_nodes = {};
            if (target_node->output(0).get_target_inputs().size() > 1) {
                // several consumers case
                for (const auto& node : target_node->output(0).get_target_inputs()) {
                    std::shared_ptr<ov::Node> target = graph_utils::get_next_node_skipping_certain(node.get_node()->shared_from_this(), is_skip_operation);
