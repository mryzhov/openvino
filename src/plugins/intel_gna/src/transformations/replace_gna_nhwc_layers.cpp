...
NGRAPH_RTTI_DEFINITION(ov::intel_gna::pass::SubstituteGNASumPool, "SubstituteGNASumPool");
...
namespace SubstituteGNASumPoolNS {
...
bool do_transformation(std::shared_ptr<ov::Node> avg_pool) {
    ...
    auto pool_new = std::make_shared<ov::intel_gna::op::GNASumPool>(transpose_before,
...
ov::intel_gna::pass::SubstituteGNASumPool::SubstituteGNASumPool() {
    MATCHER_SCOPE(SubstituteGNASumPool);
    ...
    matcher_pass_callback callback = [=](Matcher& m) {
        ...
        return SubstituteGNASumPoolNS::do_transformation(avg_pool_node);
    };
...
bool ov::intel_gna::pass::ReplaceGnaNHWCLayers::run_on_model(const std::shared_ptr<Model>& function) {
    ...
    manager.register_pass<ov::intel_gna::pass::SubstituteGNASumPool>();
