...
/**
 * @brief Substitites ngraph::AvgPool (NCHW) -> GNASumPool (NHWC)
 *
 *                              Transpose (NCHW -> NHWC)
 *                                       |
 * AvgPool (NCHW) ->               GNASumPool (NHWC)
 *                                       |
 *                              Transpose (NHWC -> NCHW)
 */
class SubstituteGNASumPool : public ov::pass::MatcherPass {
public:
    NGRAPH_RTTI_DECLARATION;
    SubstituteGNASumPool();
};
